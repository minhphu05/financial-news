"""
NER Dataset for XLM-RoBERTa Base models.

Handles subword tokenization alignment: maps word-level NER tags to
subword tokens produced by the XLM-RoBERTa tokenizer.

Strategy:
    - For each subword token, assign the tag of its source word.
    - Special tokens (CLS, SEP, PAD) receive the "O" tag label.
    - Padding positions beyond max_len use label -100 (ignored by loss).
"""
...
class NERDataset(Dataset):
    """NER Dataset with XLM-RoBERTa Base tokenization and subword alignment.

    Each sample tokenizes pre-split words using the XLM-RoBERTa tokenizer
